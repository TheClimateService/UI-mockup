...
# The following is read in by server_sector_analysis.r.
# corpTable2 <- readr::read_csv("./data/scoring_engine/nonphysical/TCSDB_structure.locations.csv.damages.allDFs.withvalues.with.nonphysical.byparentcorp.csv")
  ...
  output$selectInput_locationSector <- renderUI({
    # Sectors are defined in sheet 16 of TCSDB_structure.xlsx and are loaded by load_tcsdb.r
    ncols <- length(names(dbsheet_sectors))
    sectors <- names(dbsheet_sectors)[5:ncols]
    selectInput('inputLocationsSector',"Sectors",c("All",sectors),selectize = TRUE)
  })
  
  output$selectInput_scenarioSector <- renderUI({
    selectInput('inputScenariosSector',"Scenario",c(unique(as.character(corpTable2$ScenarioName))),selected='RCP8.5',selectize = TRUE)
  })
  
# ----------------------------
  #barByRiskFactor
# ----------------------------
  output$barByRiskFactorSector <- renderGvis({
    ...
    # ctbl_sector_all is constructed by server_sectors_analyze_dataprep.r
    corpTable2 <- ctbl_sector_all
    ncols <- length(names(dbsheet_sectors))
    sectors <- names(dbsheet_sectors)[5:ncols]
    ...
    if (input$inputLocationsSector == 'All') {
      corpTable2 <- corpTable2[which(corpTable2$RiskYear == input$sliderInputYearSector),]
    }
    ...
    if(input$riskmetric_sector=="Absolute") {
    ...
    if(input$riskmetric_sector=="Relative") {
  ...
  # treemapByTicker
  # ----------------------------
  output$treemapBySector <- renderGvis({
    #corpTable2 <- readr::read_csv("./data/scoring_engine/nonphysical/TCSDB_structure.locations.csv.damages.allDFs.withvalues.with.nonphysical.byparentcorp.csv")
    if(input$riskfactor_subset_portfolio=="Chronic physical + Carbon price") corpTable2 <- filter(corpTable2, TCFDSubCatName=="Chronic" | RiskFactorName=="Carbon pricing")
    ...
    if(input$riskmetric_sector=="Absolute") {
      values2plot <- values
      table2use <- corpTable2 %>% select(sector:ValueAtRisk)
      rfnames=corpTable2$RiskFactorName
      axisname <- "Impact ($M)"
    } # endif
    ...
    table2use <- table2use %>% mutate(unidvar = paste(table2use$RiskFactorName,table2use$sector,sep = "_")) 
    ...
    headerrows = data.frame(unidvar=tickersOfMembers, RiskFactorName=tickersOfMembers, Location=rep(input$inputLocationsSector,length(sectors)), ValueAtRisk=rep(0,length(sectors)))
    ...
    headerrows <- add_row(headerrows, unidvar=input$inputLocationsSector, RiskFactorName=input$inputLocationsSector, Location=c(NA), ValueAtRisk=c(0), .before = 1)
    ...
    gvisTreeMap(df2use, idvar = "unidvar" , parentvar = "sector", sizevar = "ValueAtRisk", colorvar = "ValueAtRisk", options = list(
...
# ----------------------------
  #barByLocation
# ----------------------------
  output$barByLocationSector <- renderPlotly({
...
#    corpTable2 <- readr::read_csv("./data/scoring_engine/nonphysical/TCSDB_structure.locations.csv.damages.allDFs.withvalues.with.nonphysical.byparentcorp.csv")
    ...
    plot_ly(table2use, x = ~sector, y = values2plot, type='bar', text=rfnames, marker = list(color = colorRampPalette(brewer.pal(11,"Spectral"))(ncorp))) %>%
...
# ----------------------------
  output$areaByTimeSector <- renderPlotly({
    #corpTable2 <- readr::read_csv("./data/scoring_engine/nonphysical/TCSDB_structure.locations.csv.damages.allDFs.withvalues.with.nonphysical.byparentcorp.csv")
    ...
    if (input$inputLocationsSector == 'All') {
      # No subsetting of corpTable2 is needed, since all time periods are shown in this plot.
      #corpTable2 <- corpTable2[which(corpTable2$RiskYear == input$sliderInputYearSector),]
    }
...
# ----------------------------
  output$stackedCorpFinImpactsPlotSector <- renderPlotly({
    #corpTable2 <- readr::read_csv("./data/scoring_engine/nonphysical/TCSDB_structure.locations.csv.damages.allDFs.withvalues.with.nonphysical.byparentcorp.csv")
    if(input$riskfactor_subset_portfolio=="Chronic physical + Carbon price") corpTable2 <- filter(corpTable2, TCFDSubCatName=="Chronic" | RiskFactorName=="Carbon pricing")
...
# ----------------------------
  #Data table
# ----------------------------
  output$corpFinImpactsSector <- DT::renderDataTable({
