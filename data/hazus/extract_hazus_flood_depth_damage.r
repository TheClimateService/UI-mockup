...
gfx_data <- subset(fl_dept, grepl("110", DmgFnId) & Cover_Class == "Bldg")
...
curve(f4damage_flood_depth, min(gfx_data$depth), max(gfx_data$depth), col="red", main="Percent Damage Vs. Flood Depth", xlab="Flood Depth (feet)", ylab="Building Damage (%)"     )
par(new=TRUE)
points(gfx_data$depth, gfx_data$damage)
