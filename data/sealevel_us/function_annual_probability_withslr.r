...
function_annual_probability_withslr = function (extremewaterLocation, sealevelProjectionLocation, returnLevel) {
...
annual_probability_withslr = slr_scenario_year
	...
	location_parameters = filter(ewl,name==extremewaterLocation) %>% select(3:8)
...
# Return level of concern in meters.
	return_level = as.numeric(returnLevel)
	#return_level = 2
...
for(i in 1:length(slrScenarios)) { 
   for(j in 1:length(slrYears)) {
	column=j+8;
	#slr_scenario_year[i,j] = filter(proj,Site==sealevelProjectionLocation) %>% filter(Scenario==slrScenarios[i]) %>% select(column) 
	temp = filter(proj,Site==sealevelProjectionLocation) %>% filter(Scenario==slrScenarios[i]) %>% select(column) 
...
# This is now done after this function is called with values in server.R.
#write.table(annual_probability_withslr,"./output/output_flood_annual_prob.csv", row.names=slrScenarios, col.names=slrYears)

#return(annual_probability_withslr)
# Return list of three key outputs.  These are assessible as function_annual_probability_withslr[[1]], [[2]], and [[3]]. 
return(list(annual_probability_withslr, slrScenarios, slrYears))
